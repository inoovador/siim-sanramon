...
mount(function () {
    $this->messages = [
        [
            'role'    => 'assistant',
            'content' => 'Soy el Asistente SIIM. Puedo explicarle cómo usar cada sección del panel, guiarlo paso a paso o resolver dudas sobre la encuesta ciudadana. ¿En qué le ayudo?',
            'time'    => now()->format('H:i'),
        ],
    ];
});
...
$send = function () {
    ...
    $this->pending = $text;
    $this->dispatch('ragchat:fetch-reply');
};

$fetchReply = function (AssistantChatUseCase $useCase) {
    ...
    try {
        ...
        $this->error = 'No pude responder en este momento. Intente nuevamente.';
    } finally {
        $this->waiting = false;
    }
};

$contextoDeFiltros = function (): string {
    return <<<CONTEXTO
    El usuario está en /panel/chat-rag y tiene estos filtros seleccionados:
    rango={$this->filters['rango']}, canal={$this->filters['canal']}, tema={$this->filters['tema']}, sentimiento={$this->filters['sentimiento']}.

    IMPORTANTE: en esta versión usted todavía NO tiene acceso al contenido de los comentarios
    ciudadanos ni a sus estadísticas. Si le piden cifras, conteos, rankings o "qué dice la gente
    sobre X", NO invente ningún número: explique que la consulta analítica sobre el corpus aún
    no está habilitada y remita a /panel/comentarios para revisar la lista filtrable o a
    /panel/reportes para exportar los datos. Sí puede ayudar con el uso del sistema, los flujos
    del panel, los roles y la encuesta ciudadana.
    CONTEXTO;
};
...
<div
    ...
    x-data
    @ragchat:fetch-reply.window="setTimeout(() => $wire.fetchReply(), 80)"
>
    ...
    <div class="flex gap-4 items-start">
            ...
            <div class="card">
                ...
                <div class="flex flex-col gap-2">
                    @foreach([
                        ...
                        '¿Cómo filtro los comentarios?',
                        '¿Cómo exporto un reporte en PDF?',
                        '¿Cómo difundo la encuesta ciudadana?',
                    ] as $atajo)
                        <button
        ...
        <div class="flex-1 card flex flex-col overflow-hidden" style="height: calc(100vh - 220px);">
            ...
            <div class="bg-brand-river/10 border border-brand-river/20 rounded-lg px-4 py-2.5 mb-3 flex items-center gap-2 text-xs text-brand-river flex-shrink-0">
                ...
                Asistente conectado. Responde dudas sobre el uso del sistema; la consulta analítica sobre el corpus de comentarios se habilita en F6.
            </div>

            @if ($error)
                <div class="bg-brand-clay/10 border border-brand-clay/20 text-brand-clay text-xs px-4 py-2.5 rounded-lg mb-3 flex-shrink-0">
                    {{ $error }}
                </div>
            @endif
                ...
                @foreach ($messages as $msg)
                    ...
                    @else
                        <div class="flex gap-2">
                            ...
                            <div class="max-w-[75%]">
                                <div class="bg-brand-mist border border-brand-canopy/10 rounded-2xl rounded-tl-sm px-4 py-3 text-sm leading-relaxed text-ink-deep whitespace-pre-wrap">{{ $msg['content'] }}</div>
